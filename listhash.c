...
{
    hashtab_node **htables;                                                 // hash table buckets 就是数组里面存着hashtab_node类型的指针
    int size;                                                               // max size
    int nel;                                                                // elemets count
    int (*hash_value)(struct _hashtab *h, const void *key);                 // hash function
    int (*keycomp)(struct _hashtab *h, const void *key1, const void *key2); // hash key compare func, when the hashvalue equal
...
hashtab *hashtab_create(int size, hash_key_func hash_value, keycmp_func keycomp, hash_node_free_func hash_node_free);
...
// 下面是实现 上面的额可以提取到 .h 文件

hashtab *hashtab_create(int size, hash_key_func hash_value, keycmp_func keycomp, hash_node_free_func hash_node_free)
{
    hashtab *h = NULL;
    int i = 0;
    if ((size < 0) || (hash_value == NULL) || (keycomp == NULL))
        return NULL;

    h = malloc(sizeof(hashtab));
    assert(h != NULL);

    h->htables = malloc(size * sizeof(hashtab_node *));
    assert(h->htables != NULL);
...
int hashtab_insert(hashtab *h, void *key, void *data)
{
    ...
    //如果key和当前的key比对一致 直接返回 数据已经存在
    if (cur != NULL && h->keycomp(h, key, cur->key) == 0)
        return 2;

    newnode = malloc(sizeof(hashtab_node));
    if (newnode == NULL)
        return 3;
...
hashtab_node *hashtab_delete(hashtab *h, void *key)
{
    ...
    hvalue = h->hash_value(h, key);
    cur = h->htables[hvalue];
    // hash桶中元素是从小到大排列的要找到插入的位置
    while (cur != NULL && h->keycomp(h, key, cur->key) >= 0)
    {
        if (h->keycomp(h, key, cur->key) == 0)
        {
            if (prev == NULL)
            {
...
int hashtab_havalue(hashtab *h, const void *key)
{
    return siample_hash(key) % h->size;
}

int hashtab_keycomp(hashtab *h, const void *key1, const void *key2)
{
...
int main(int argc, char const *argv[])
{
    ...
    h = hashtab_create(50, hashtab_havalue, hashtab_keycomp, hashtab_node_free);
    assert(h != NULL);

    while (1)
    {
        p = malloc(sizeof(struct test_node));
        assert(p != NULL);
        printf("\r\n 请输入key value，当key等于\"quit\"时退出");
        scanf("%s", p->key);
        scanf("%s", p->data);
